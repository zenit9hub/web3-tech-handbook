...
앞선 두 문서가 Web3를 `검증 가능한 신뢰 인프라`로 재정의했다면, 이 문서는 그 추상적 설명을 시스템 메커니즘 수준으로 내린다. 계정, 상태, 트랜잭션, 확정성을 한 문서에서 연결해, Web3의 신뢰가 실제로 어떻게 구현되는지 설명한다.
...
실무에서는 `확정성 = 리스크 기반 운영 기준`으로 이해하는 편이 빠르다.

## 선행 개념

- [신뢰 인프라의 진화: Bitcoin에서 RWA까지](/foundations/)
