...
- 이 문서는 발표에서 시장 전환의 맥락을 잡는 장으로 적합하다.
- 세부 수치는 발표 직전 최신성 검증이 필요하지만, 단계 구분 자체는 장기적으로 재사용 가능하다.
- 청중이 비개발자라면 `문제가 어떻게 바뀌었는가` 중심으로, 개발자라면 `어떤 레이어가 추가되었는가` 중심으로 설명하는 편이 좋다.
